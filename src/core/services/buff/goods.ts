...
export const parseGoodsList = (goodsList: IBuffGoodsItem[]): IGoods[] => goodsList.map((g) => ({
    ...
    diff_price: Number(g.sell_min_price) / Number(g.goods_info.steam_price_cny), // 倍数
    original_discount_price: Number(g.sell_min_price) * 1.15 / Number(g.goods_info.steam_price_cny) * 100, // 原始折扣价
    original_profit: `${((Number(g.sell_min_price) * (1 - (g.game === "dota2" ? 0.018 : 0.025)) - Number(g.goods_info.steam_price_cny) * 0.82)) / (Number(g.goods_info.steam_price_cny) * 0.82) * 100}%`, // 原始转回利润
